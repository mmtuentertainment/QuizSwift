...
must_haves:
  truths:
    ...
    - "Questions are verified against source using vector similarity"
    - "Low-confidence questions are stored but flagged with warning (not deleted)"
    - "Users see flagged questions with warnings but can still review/edit them"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/lib/ai/verify-grounding.ts"
      ...
      via: "vector similarity query using chunk embeddings"
      pattern: "sourceChunk.*embedding"
    - from: "src/inngest/functions/extract-questions.ts"
      to: "prisma.extractedQuestion"
      via: "question storage with verified/flagged fields"
...
Purpose: Extract quiz questions from source text using LLMs with structured output, then verify each question is grounded in the source material using vector similarity with pre-computed chunk embeddings. Store ALL questions but flag unverified ones with warnings for teacher review.
Output: Complete AI extraction pipeline with dual-tier provider support (Ollama/OpenAI), structured question schema, and grounding verification that flags (not deletes) low-confidence extractions.
</objective>
...
<task type="auto">
  ...
  <action>
1. Install additional AI providers (if not already installed from 02-03):
```bash
npm install ollama-ai-provider zod
```
...
4. Update src/lib/ai/index.ts to include all exports:
```typescript
...
5. Add OpenAI env var to .env.local.example (if not already present):
```
# AI Providers
OPENAI_API_KEY=
```
...
- .env.local.example includes OPENAI_API_KEY
  ...
  <done>
Dual-tier AI providers configured. Question extraction uses structured Zod schema with generateObject. Extraction prompt enforces extraction-only behavior with source quote requirement.
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement grounding verification using chunk embeddings</name>
  <files>src/lib/ai/verify-grounding.ts</files>
  <action>
Create src/lib/ai/verify-grounding.ts:
```typescript
...
- Vector similarity query uses chunk embeddings from 02-03
- Fallback text-based matching works when embeddings unavailable
  </verify>
  <done>
Grounding verification uses vector similarity with pre-computed chunk embeddings. Falls back to text matching as edge case. Questions below threshold flagged as weak or ungrounded (not deleted).
  </done>
</task>

<task type="auto">
  <name>Task 3: Create Inngest function for AI extraction with flag-not-reject behavior</name>
...
import { verifyGrounding, GROUNDING_THRESHOLD, WARNING_THRESHOLD } from '@/lib/ai/verify-grounding';
...
- ALL questions stored (verified AND flagged)
- Flagged questions have flagReason explaining why
- Document status transitions to completed with flagged count warning
  </verify>
  <done>
Inngest extraction job processes chunks with AI, verifies grounding using vector similarity, stores ALL questions with verification status. Flagged questions are stored with warning reasons - NOT deleted. Teachers see all questions with flagged ones marked for review. Full pipeline: upload -> process -> extract -> complete.
  </done>
</task>
...
2. Extraction logic:
   - extractQuestionsFromChunk uses generateObject with structured schema
   - Questions include sourceQuote field
   - Batch extraction handles errors gracefully

3. Grounding verification:
   - verifyGrounding uses vector similarity with pre-computed chunk embeddings
   - Fallback to text matching when no embeddings (edge case)
   - Results include similarity score and status

4. Flag-not-reject behavior (CONT-05):
   - ALL questions stored in database
   - verified: true/false based on GROUNDING_THRESHOLD
   - flagged: true when verified=false
   - flagReason: explains why for teacher review
   - Document shows count of flagged questions
...
- src/lib/ai/extract-questions.ts extracts with Zod schema using generateObject
- src/lib/ai/verify-grounding.ts verifies using vector similarity (threshold 0.85)
- src/inngest/functions/extract-questions.ts completes extraction pipeline
- ALL questions stored (verified AND flagged - no deletion)
- Flagged questions have flagReason explaining verification failure
- Document shows flagged count in errorMessage field
